...
struct ContactsViewModel {
    ...
    static var shared = ContactsViewModel()
    private init() {}
    ...
    func getContacts() -> [CNContact]? { //  ContactsFilter is Enum find it below
        ...
        do {
            ...
            print("Error fetching containers: \(e.localizedDescription)")
            return nil
        }
    ...
    func getPhoneContacts() -> [PhoneContact]? {
        ...
        var phoneContacts: [PhoneContact]? = nil
     ...
     func getContactsWithAccount() {
        ...
        let currentUser = AuthViewModel.shared.currentUser
         ...
         for i in 0..<Int(ceil(Double(contacts.count)/10)) {
            ...
            let startIndex = i * 10
            let contactGroup = contacts[startIndex..<startIndex + min(10, contacts.count - startIndex)]
            
            var numbers = [String]()
            contactGroup.forEach({
                
                if let number = $0.phoneNumberNoPrefix.first {
